...
library MerkleTree {
    ...
    function computeRoot(bytes32[] memory leaves) internal pure returns (bytes32 root) {
        uint8 treeDepth = computeMinimalTreeDepth(leaves.length);

        uint256 totalLeaves = 1 << treeDepth; // 2^treeDepth

        // Create array of full leaf set with padding if necessary
        bytes32[] memory nodes = new bytes32[](totalLeaves);
        ...
        uint256 currentSize = totalLeaves;
        while (currentSize > 1) {
            currentSize /= 2;
